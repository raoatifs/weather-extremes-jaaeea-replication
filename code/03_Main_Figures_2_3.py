...
Inputs (read from the Data Files folder, one level up from this script):
  Corndata.dta
  combined_NorESM2-MM_2021-2100.dta
...
def make_figure3():
    hist = pd.read_stata(DATA_DIR/"Corndata.dta", convert_categoricals=False)
    fut  = pd.read_stata(DATA_DIR/"combined_NorESM2-MM_2021-2100.dta", convert_categoricals=False)
    for c in ['fips', 'year']:
